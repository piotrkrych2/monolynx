...
async def verify_api_key(
    x_opensentry_key: str = Header(...),
    db: AsyncSession = Depends(get_db),
) -> Project:
    """FastAPI dependency -- walidacja API key z cache."""
    now = time.time()
    if x_opensentry_key in _api_key_cache:
        project_id, cached_at = _api_key_cache[x_opensentry_key]
    ...
    result = await db.execute(
        select(Project).where(
            Project.api_key == x_opensentry_key,
    ...
    _api_key_cache[x_opensentry_key] = (project.id, now)
    return project
