...
class Config:
    ...
    def from_django_settings(cls, settings: object) -> Config:
        dsn = getattr(settings, "OPEN_SENTRY_DSN", None)

        if dsn:
            server_url, api_key = cls._parse_dsn(dsn)
        else:
            server_url = getattr(settings, "OPEN_SENTRY_URL", None)
        ...
        return cls(
            ...
            api_key=api_key,
            environment=getattr(settings, "OPEN_SENTRY_ENVIRONMENT", "production"),
            release=getattr(settings, "OPEN_SENTRY_RELEASE", ""),
        )
