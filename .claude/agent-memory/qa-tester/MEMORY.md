...
- MON-87 burndown.py/activity.py: testy na realnej bazie (db_session, bez mocków ORM). `Ticket.updated_at`/`ActivityLog.created_at` maja `server_default=func.now()` ale jawne przypisanie PRZED pierwszym flush (INSERT) nadpisuje default -- uzywaj do deterministycznych dat w testach zamiast time.sleep
- Weryfikacja "serwis robi flush nie commit" (regula db-commit.md): otworz NIEZALEZNE polaczenie przez `engine` fixture (session-scoped) i sprawdz ze wiersz niewidoczny z innego polaczenia -- to jedyny pewny sposob odroznic flush od commit w tescie (bo w tej samej sesji/transakcji flush juz "wyglada" jak sukces - ma ID)
- Symulacja `ProgrammingError` (brakujaca tabela) w testach integracyjnych: monkeypatch.setattr(db_session, "execute"/"flush", wrapper) z licznikiem `state["called"]` tak zeby TYLKO pierwsze wywolanie rzucalo blad, kolejne delegowaly do oryginalnej metody (`original = getattr(session, method)` przed patchem) -- pozwala testowac pojedynczy endpoint bez psucia reszty przeplywu w tym samym tescie
